...
write(int handle, void *buffer, unsigned int len) {
	if (handle == 1 || handle == 2) {
		unsigned int lenPrinted = 0;
		while (lenPrinted < len) {
                    struct rpcDataBuffer rpcBuf;
                    if(!allocRPCBuffer(&rpcBuf))
                        return (lenPrinted > 0) ? lenPrinted : -1;

                    unsigned int currLen = len - lenPrinted;
                    if(currLen > MAX_RPC_SIZE)
                        currLen = MAX_RPC_SIZE;
                    memcpy(rpcBuf.data, buffer + lenPrinted, currLen);

                    if(!doRPCCall(&rpcBuf, NULL, 0, 0x10, 0, 0))
                       return (lenPrinted > 0) ? lenPrinted : -1;
